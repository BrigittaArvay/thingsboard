...
import {
  ...
  flotSettingsSchema,
  IotopFlotAxisOptions,
  IotopFlotHoverInfo,
  IotopFlotKeySettings,
  IotopFlotPlotAxis,
  IotopFlotPlotDataSeries,
  IotopFlotPlotItem,
  IotopFlotSeries,
  IotopFlotSeriesHoverInfo,
  IotopFlotSettings,
  IotopFlotThresholdKeySettings,
  IotopFlotThresholdMarking,
  IotopFlotTicksFormatterFunction,
  TooltipValueFormatFunction
} from './flot-widget-iotop.models';
...
export class IotopFlot {

  private readonly utils: UtilsService;

  private settings: IotopFlotSettings;

  private readonly tooltip: JQuery<any>;

  private readonly yAxisTickFormatter: IotopFlotTicksFormatterFunction;
  private readonly yaxis: IotopFlotAxisOptions;
  private readonly xaxis: IotopFlotAxisOptions;
  private yaxes: Array<IotopFlotAxisOptions>;
  ...
  private predefinedThresholds: IotopFlotThresholdMarking[];
  ...
  constructor(private ctx: WidgetContext, private readonly chartType: ChartType) {
    ...
    this.settings = ctx.settings as IotopFlotSettings;
        ...
        if (this.settings.yaxis.ticksFormatter && this.settings.yaxis.ticksFormatter.length) {
          try {
            this.yaxis.ticksFormatterFunction = new Function('value',
                                               this.settings.yaxis.ticksFormatter) as IotopFlotTicksFormatterFunction;
          } catch (e) {
  ...
  private init($element: JQuery<any>, subscription: IWidgetSubscription) {
    ...
    const colors: string[] = [];
    this.yaxes = [];
    const yaxesMap: {[units: string]: IotopFlotAxisOptions} = {};
    const predefinedThresholds: IotopFlotThresholdMarking[] = [];
    ...
    for (let i = 0; i < this.subscription.data.length; i++) {
      const series = this.subscription.data[i] as IotopFlotSeries;
      ...
      if (this.yaxis) {
        ...
        let yaxis: IotopFlotAxisOptions;
          ...
          for (const threshold of keySettings.thresholds) {
            if (threshold.thresholdValueSource === 'predefinedValue' && isFinite(threshold.thresholdValue)) {
              ...
              const dataKey: DataKey = {
                ...
                } as IotopFlotThresholdKeySettings,
                _hash: Math.random()
              };
              if (datasource) {
            ...
            if (axisVisibilityChanged) {
              ...
              this.yaxes.forEach((yaxis) => {
                ...
                for (let k = 0; k < yaxis.keysInfo.length; k++) {
                  if (yaxis.keysInfo[k]) {
                    (this.subscription.data[k] as IotopFlotSeries).yaxis = newIndex;
                  }
                }
  ...
  private createYAxis(keySettings: IotopFlotKeySettings, units: string): IotopFlotAxisOptions {
    ...
    if (keySettings.axisTicksFormatter && keySettings.axisTicksFormatter.length) {
      try {
        yaxis.ticksFormatterFunction = new Function('value', keySettings.axisTicksFormatter) as IotopFlotTicksFormatterFunction;
      } catch (e) {
    ...
    data.forEach((keyData) => {
      if (keyData && keyData.data && keyData.data[0]) {
        ...
        if (isFinite(attrValue)) {
          const settings: IotopFlotThresholdKeySettings = keyData.dataKey.settings;
  ...
  private generateThreshold(existingThresholds: IotopFlotThresholdMarking[], yaxis: number, lineWidth: number,
                            color: string, defaultColorIndex: number, thresholdValue: number) {
    const marking: IotopFlotThresholdMarking = {};
    let markingYAxis;
  ...
  private substituteLabelPatterns(series: IotopFlotSeries, seriesIndex: number) {
    ...
    for (const sourcesData of seriesLabelPatternsSourcesData) {
      const keyData = sourcesData;
      if (keyData && keyData.data && keyData.data[0]) {
        const attrValue = keyData.data[0][1];
  ...
  private seriesInfoDivFromInfo(seriesHoverInfo: IotopFlotSeriesHoverInfo, seriesIndex: number): string {
  ...
  private formatPieTooltip(item: IotopFlotPlotItem): string {
  ...
  private formatChartTooltip(hoverInfo: IotopFlotHoverInfo[], seriesIndex: number): string {
    let content = '';
    if (this.tooltipIndividual) {
      let seriesHoverArray: IotopFlotSeriesHoverInfo[];
  ...
  private formatYAxisTicks(value: number, axis?: IotopFlotPlotAxis): string {
  ...
  private enableMouseEvents() {
    ...
    this.$element.on('plothover', this.flotHoverHandler);
    this.$element.on('plotselected', this.flotSelectHandler);
    this.$element.on('dblclick', this.dblclickHandler);
    ...
    this.$element.on('plotclick', this.flotClickHandler);
  }
  ...
  private disableMouseEvents() {
    ...
    this.$element.off('plothover', this.flotHoverHandler);
    this.$element.off('plotselected', this.flotSelectHandler);
    this.$element.off('dblclick', this.dblclickHandler);
    ...
    this.$element.off('plotclick', this.flotClickHandler);
  }

  private onFlotHover(e: any, pos: JQueryPlotPoint, item: IotopFlotPlotItem) {
    if (!this.plot) {
      return;
    }
    if (!this.tooltipIndividual || item) {
      ...
      let tooltipHtml;
      let hoverInfo: IotopFlotHoverInfo[];
  ...
  private onFlotClick(e: any, pos: JQueryPlotPoint, item: IotopFlotPlotItem) {
    if (!this.plot) {
      return;
    }
  ...
  private getHoverInfo(seriesList: IotopFlotPlotDataSeries[], pos: JQueryPlotPoint): IotopFlotHoverInfo[] {
    let i: number;
    let series: IotopFlotPlotDataSeries;
    ...
    let hoverData: IotopFlotSeriesHoverInfo;
    let value: any;
    let lastValue: any;
    let minDistanceHistorical: number;
    const results: IotopFlotHoverInfo[] = [{
      seriesHover: []
    }];
    if (this.settings.comparisonEnabled) {
      results.push({
  ...
  private findHoverIndexFromData(posX: number, series: IotopFlotPlotDataSeries): number {
    let lower = 0;
  ...
  private findHoverIndexFromDataPoints(posX: number, series: IotopFlotPlotDataSeries, last: number): number {
  ...
  private onPieSliceClick($event: any, item: IotopFlotPlotItem) {
